...
wiced_result_t hello_client_management_cback( wiced_bt_management_evt_t event,  wiced_bt_management_evt_data_t *p_event_data)
{
    ...
    WPRINT_BT_APP_INFO(("hello_client_management_cback: %x\n", event ));
    ...
    case BTM_ENCRYPTION_STATUS_EVT:
    {
    	...
    	WPRINT_BT_APP_INFO(( "encryption status: res %d\n", p_status->result));
    ...
    default:
    	WPRINT_BT_APP_INFO(("Unknown BTM event %d\n",event));
    	break;
    }
...
wiced_bt_gatt_status_t hello_client_gatt_callback( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_data)
{
    ...
    WPRINT_BT_APP_INFO(( "hello_client_gatt_callback event %d \n", event ));
    ...
    switch( event )
    {
        case GATT_CONNECTION_STATUS_EVT:
            ...
            {
            	WPRINT_APP_INFO(("GATT_CONNECTION_STATUS_EVT connected\n"));
                result = hello_client_gatt_connection_up( &p_data->connection_status );
            }
            else
            {
            	WPRINT_APP_INFO(("GATT_CONNECTION_STATUS_EVT Disconnected\n"));
        ...
        case GATT_OPERATION_CPLT_EVT:
        		WPRINT_APP_INFO(("GATT_OPERATION_CPLT_EVT\n"));
...
{
    WPRINT_BT_APP_INFO(( "hello_client_connection_down <%X:%X:%X:%X:%X:%X>\n", p_conn_status->bd_addr[0], p_conn_status->bd_addr[1], p_conn_status->bd_addr[2],
            p_conn_status->bd_addr[3], p_conn_status->bd_addr[4], p_conn_status->bd_addr[5] ));
...
wiced_bt_gatt_status_t hello_client_gatt_op_comp_cb( wiced_bt_gatt_operation_complete_t *p_data )
{
    ...
    WPRINT_BT_APP_INFO(("hello_client_gatt_op_comp_cb conn %d op %d st %d\n", p_data->conn_id, p_data->op, p_data->status ));

    switch ( p_data->op )
    {
    case GATTC_OPTYPE_READ:
        WPRINT_BT_APP_INFO(( "read_rsp status:%d\n", p_data->status ));
        break;

    case GATTC_OPTYPE_WRITE:
        WPRINT_BT_APP_INFO(( "write_rsp status:%d\n", p_data->status ));
        break;

    case GATTC_OPTYPE_CONFIG:
        WPRINT_BT_APP_INFO(( "peer mtu:%d\n", p_data->response_data.mtu ));
        break;
    }
    return WICED_BT_GATT_SUCCESS;
}
...
void hello_client_scan_result_cback( wiced_bt_ble_scan_results_t *p_scan_result, uint8_t *p_adv_data )
{
    ...
    if ( p_scan_result )
    {
        ...
        WPRINT_BT_APP_INFO(("Found Motor Service\n"));
        ...
        WPRINT_BT_APP_INFO(( "scan off status %d\n", status ));
        ...
        WPRINT_BT_APP_INFO(( "wiced_bt_gatt_connect status %d\n", ret_status ));
    }
    else
    {
        WPRINT_BT_APP_INFO(( "Scan completed.\n" ));
    }
}
...
void hello_client_write_motor_m1 ( uint8_t val )
{
	WPRINT_APP_INFO(("Writing Motor Message M1 %d\n",val));
    ...
    if ( p_write )
    {
        ...
        status = wiced_bt_gatt_send_write ( g_hello_client.conn_id, GATT_WRITE_NO_RSP, p_write );

        WPRINT_BT_APP_INFO(("Wrote Motor M1 %d\n", status));
    }
}

void hello_client_write_motor_m2 ( uint8_t val )
{
	WPRINT_APP_INFO(("Writing Motor Message M2 %d\n",val));
    wiced_bt_gatt_status_t status;
    wiced_bt_gatt_value_t write_buffer;
    wiced_bt_gatt_value_t *p_write = &write_buffer;
    if ( p_write )
    {
        ...
        WPRINT_BT_APP_INFO(("Wrote Motor M2 %d\n", status));
    }
